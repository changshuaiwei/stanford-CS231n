...
##########################################
# helper layer with batch normalization  #
##########################################
...
def conv_batch_relu_pool_forward(x, w, b, gamma, beta, conv_param, bn_param, pool_param):
  ...
  """
  a, conv_cache = conv_forward_fast(x, w, b, conv_param)
  aa, batch_cache = spatial_batchnorm_forward(a, gamma, beta, bn_param)
...
def conv_batch_relu_pool_backward(dout, cache):
  ...
  ds = max_pool_backward_fast(dout, pool_cache)
  daa = relu_backward(ds, relu_cache)
  da, dgamma, dbeta = spatial_batchnorm_backward(daa, batch_cache)
  dx, dw, db = conv_backward_fast(da, conv_cache)
...
def affine_batch_relu_forward(x, w, b, gamma, beta, bn_param):
  ...
  """
  a, fc_cache = affine_forward(x, w, b)
  a_bt, bt_cache = batchnorm_forward(a, gamma, beta, bn_param)
  out, relu_cache = relu_forward(a_bt)
...
def affine_batch_relu_backward(dout, cache):
  ...
  da_bt = relu_backward(dout, relu_cache)
  da, dgamma, dbeta = batchnorm_backward_alt(da_bt, bt_cache)
  dx, dw, db = affine_backward(da, fc_cache)
  return dx, dw, db, dgamma, dbeta
